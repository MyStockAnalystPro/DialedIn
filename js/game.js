...
const Game = (() => {
  ...
  function buy(id) {
    ...
    if (Store.s.coins < item.price) { toast("Not enough coins yet — keep grinding! 🪙", "bad"); AudioFX.play("fail"); return; }
    Store.s.coins -= item.price;
    if (item.type === "reward") {
      AudioFX.play("coin");
      ...
      if (Store.s.shopOwned.includes(id)) { toast("Already owned!"); Store.s.coins += item.price; return; }
